...
<body>
    <Header />
    <div v-for="(item, idx) in state.sections" :key="idx" class="section">
...
.icon-arrow {fill: black; height: 15px; width: 15px; position: absolute; z-index: 1;}
.icon-circle {fill:rgb(235, 235, 235); height: 35px; width: 35px;}
.right:hover .icon-circle {transform: scale(1.1); fill:rgb(218, 218, 218); transition: transform 0.3s ease, fill 0.3s ease;}
.right:hover .icon-arrow {transform: scale(1.1); fill:black; transition: transform 0.3s ease, fill 0.3s ease;}
